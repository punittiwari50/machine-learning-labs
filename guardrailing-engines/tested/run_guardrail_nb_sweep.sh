...
mkdir -p guardrailing-engines/tested

PASS=0
FAIL=0

for nb in guardrailing-engines/*.ipynb; do
    base="${nb##*/}"
    base="${base%.ipynb}"
    ...
    echo "[RUN] ${nb}"
    if jupyter nbconvert --to notebook --execute "${nb}" --output "${out}" --output-dir guardrailing-engines/tested >"${log}" 2>&1; then
        echo "[PASS] ${nb}"
        PASS=$((PASS + 1))
    else
        echo "[FAIL] ${nb}"
